                    ...
                    <div class="col-md-12 grid-margin stretch-card">
                      <div class="card" style="background-color: #fff;">
                        <div class="card-body">
                          ...
                          <form class="forms-sample" action="{{url('/admin/edit_admin_list/'.$edit->id)}}" method="POST" role="form" enctype="multipart/form-data">
                            @csrf
                            <div class="form-group row mb-3">
                                ...
                                <select class="js-example-basic-single form-select form-select-white mb-3 text-dark bg-outline-dark border-3" id="parent" name="parent" style="width:100%" >
                                    {{-- @if($edit->role == "SubAdmin" || $edit->role == "SuperAdmin") --}}
                                    <option class="text-dark" value="SubAdmin" @php echo ($edit->role == 'SubAdmin') ? 'selected':'' @endphp>SubAdmin</option>
                                    <option class="text-dark" value="Administrator" @php echo ($edit->role == 'Administrator') ? 'selected':'' @endphp>Administrator</option>
                                    {{-- @endif --}}
                                </select>
                                </div>
                            </div>
                            <div class="form-group row ">
                              ...
                              <div class="col-sm-9 mb-3">
                                <input type="text" multiple class="form-select bg-outline-dark border-3" name="name" id="name" value="{{$edit->name}}" placeholder="Enter a name">
                              </div>
                            </div>
                            <div class="form-group row ">
                              ...
                              <div class="col-sm-9 mb-3">
                                <input type="email" multiple class="form-select bg-outline-dark border-3" name="email" id="email" value="{{$edit->email}}" placeholder="Enter a Email">
                              </div>
                            </div>
                            <div class="form-group row">
                            ...
                            <div class="form-group row ">
                                ...
                                <div class="col-sm-9 mb-3 mm">
                                  @php $module_1 = explode('|',$edit->accountAccessModule);@endphp
                                    @foreach($module as $row)
                                      <div class="form-check-inline h5 text-dark">
                                        @if(in_array($row->module_name,$module_1))
                                          <input class="form-check-input bg-outline-dark border-3" type="checkbox" id="inlineCheckbox1" name="model[]" value="{{$row->module_name}}" checked>
                                          <label class="form-check-label" for="image_1">{{$row->module_name}}</label>
                                        @else
                                          <input class="form-check-input bg-outline-dark border-3" type="checkbox" id="inlineCheckbox2" name="model[]" value="{{$row->module_name}}">
                                          <label class="form-check-label" for="image_1"> {{$row->module_name}}</label>
                                        @endif
                                      </div>
                                    @endforeach
                            ...
                            <div class="form-group row ">
                                <label class="col-sm-3 col-form-label h2 text-dark" style="padding-left: 10px">Account Previleges <span style="color:red;">&nbsp;*</span></label>
                                <div class="col-sm-9 mb-3 mm">
                                    @php $array= array('view','edit','delete','add') @endphp
                                    @php $privileges = explode('|',$edit->accountAccessPrivilege);@endphp
                                    @foreach($array as $arr)
                                     <div class="form-check-inline h5 text-dark">
                                      @if(in_array($arr, $privileges))
                                      <input class="form-check-input bg-outline-dark border-3" type="checkbox" id="inlineCheckbox1" name="privileges[]" value="{{$arr}}" checked>
                                        <label class="form-check-label" for="image_1">
                                        {{$arr}}
                                      </label>
                                      @else
                                        <input class="form-check-input bg-outline-dark border-3" type="checkbox" id="inlineCheckbox1" name="privileges[]" value="{{$arr}}">
                                        <label class="form-check-label" for="image_1">
                                        {{$arr}}
                                        </label>
                                      @endif
